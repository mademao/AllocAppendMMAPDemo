...
#define SGI_RSSWReturnType(type) type

/// A macro for wrapping arguments of the swizzled method.
#define SGI_RSSWArguments(arguments...) _SGI_RSSWArguments(arguments)

/// A macro for wrapping the replacement code for the swizzled method.
#define SGI_RSSWReplacement(code...) code

/// A macro for casting and calling original implementation.
/// May be used only in SGI_RSSwizzleInstanceMethod or SGI_RSSwizzleClassMethod macros.
#define SGI_RSSWCallOriginal(arguments...) _SGI_RSSWCallOriginal(arguments)
...
/**
 ...
 SGI_RSSwizzleInstanceMethod(classToSwizzle,
    @selector(calculate:),
    SGI_RSSWReturnType(int),
    SGI_RSSWArguments(int number),
    SGI_RSSWReplacement({
        // Calling original implementation.
        int res = SGI_RSSWCallOriginal(number);
 ...
 Swizzling frequently goes along with checking whether this particular class (or one of its superclasses) has been already swizzled. Here the `SGI_RSSwizzleMode` and `key` parameters can help. See +[SGI_RSSwizzle swizzleInstanceMethod:inClass:newImpFactory:mode:key:] for details.
 ...
 @param SGI_RSSWReturnType The return type of the swizzled method wrapped in the SGI_RSSWReturnType macro.

 @param SGI_RSSWArguments The arguments of the swizzled method wrapped in the SGI_RSSWArguments macro.

 @param SGI_RSSWReplacement The code of the new implementation of the swizzled method wrapped in the SGI_RSSWReplacement macro.

 @param SGI_RSSwizzleMode The mode is used in combination with the key to indicate whether the swizzling should be done for the given class. You can pass 0 for SGI_RSSwizzleModeAlways.

 @param key The key is used in combination with the mode to indicate whether the swizzling should be done for the given class. May be NULL if the mode is SGI_RSSwizzleModeAlways.
 ...
 */
#define SGI_RSSwizzleInstanceMethod(classToSwizzle, \
    selector,                                       \
    SGI_RSSWReturnType,                             \
    SGI_RSSWArguments,                              \
    SGI_RSSWReplacement,                            \
    ...
    _RSSwizzleInstanceMethod(classToSwizzle,        \
        selector,                                   \
        SGI_RSSWReturnType,                         \
        _SGI_RSSWWrapArg(SGI_RSSWArguments),        \
        _SGI_RSSWWrapArg(SGI_RSSWReplacement),      \
        SGI_RSSwizzleMode,                          \
        key)
...
/**
 ...
 SGI_RSSwizzleClassMethod(classToSwizzle,
    @selector(calculate:),
    SGI_RSSWReturnType(int),
    SGI_RSSWArguments(int number),
    SGI_RSSWReplacement({
 ...
 */
#define SGI_RSSwizzleClassMethod(classToSwizzle, \
    selector,                                    \
    SGI_RSSWReturnType,                          \
    SGI_RSSWArguments,                           \
    SGI_RSSWReplacement)                         \
 ...
 */
typedef void (*SGI_RSSwizzleOriginalIMP)(void /* id, SEL, ... */);

/**
 SGI_RSSwizzleInfo is used in the new implementation block to get and call original implementation of the swizzled method.
 */
@interface SGI_RSSwizzleInfo : NSObject
 ...
 */
- (SGI_RSSwizzleOriginalIMP)getOriginalImplementation;
 ...
 */
typedef id (^SGI_RSSwizzleImpFactoryBlock)(SGI_RSSwizzleInfo *swizzleInfo);

typedef NS_ENUM(NSUInteger, SGI_RSSwizzleMode) {
    /// SGI_RSSwizzle always does swizzling.
    SGI_RSSwizzleModeAlways = 0,
    /// SGI_RSSwizzle does not do swizzling if the same class has been swizzled earlier with the same key.
    SGI_RSSwizzleModeOncePerClass = 1,
    /// SGI_RSSwizzle does not do swizzling if the same class or one of its superclasses have been swizzled earlier with the same key.
    ...
    SGI_RSSwizzleModeOncePerClassAndSuperclasses = 2
};

@interface SGI_RSSwizzle : NSObject
...
/**
 ...
 [SGI_RSSwizzle
    ...
    newImpFactory:^id(SGI_RSSWizzleInfo *swizzleInfo) {
 ...
 [SGI_RSSwizzle
    ...
    mode:SGI_RSSwizzleModeOncePerClassAndSuperclasses
    key:key];
...
+ (BOOL)swizzleInstanceMethod:(SEL)selector
                ...
                newImpFactory:(SGI_RSSwizzleImpFactoryBlock)factoryBlock
                         mode:(SGI_RSSwizzleMode)mode
                          key:(const void *)key;
...
#define _SGI_RSSWWrapArg(args...) args

#define _SGI_RSSWDel2Arg(a1, a2, args...) a1, ##args
#define _SGI_RSSWDel3Arg(a1, a2, a3, args...) a1, a2, ##args

// To prevent comma issues if there are no arguments we add one dummy argument
// and remove it later.
#define _SGI_RSSWArguments(arguments...) DEL, ##arguments
...
#define _RSSwizzleInstanceMethod(classToSwizzle,                                                           \
    ...
    [SGI_RSSwizzle                                                                                         \
        swizzleInstanceMethod:selector                                                                     \
                ...
                newImpFactory:^id(SGI_RSSwizzleInfo *swizzleInfo) {                                        \
                    SGI_RSSWReturnType (*originalImplementation_)(_SGI_RSSWDel3Arg(__unsafe_unretained id, \
                        SEL,                                                                               \
                        SGI_RSSWArguments));                                                               \
                    SEL selector_ = selector;                                                              \
                    return ^SGI_RSSWReturnType(_SGI_RSSWDel2Arg(__unsafe_unretained id self,               \
                        SGI_RSSWArguments)) {                                                              \
                        SGI_RSSWReplacement                                                                \
                    };                                                                                     \
                }                                                                                          \
                         mode:SGI_RSSwizzleMode                                                            \
                          key:KEY];
...
#define _SGI_RSSWCallOriginal(arguments...)          \
