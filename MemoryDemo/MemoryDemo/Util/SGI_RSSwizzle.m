...
static BOOL blockIsAnImpFactoryBlock(id block) {
    const char *blockType = blockGetType(block);
    SGI_RSSwizzleImpFactoryBlock dummyFactory = ^id(SGI_RSSwizzleInfo *swizzleInfo) {
        return nil;
    };
...
#pragma mark └ SGI_RSSwizzleInfo
typedef IMP (^SGI_RSSWizzleImpProvider)(void);

@interface SGI_RSSwizzleInfo ()
@property (nonatomic, copy) SGI_RSSWizzleImpProvider impProviderBlock;
@property (nonatomic, readwrite) SEL selector;
@end

@implementation SGI_RSSwizzleInfo

- (SGI_RSSwizzleOriginalIMP)getOriginalImplementation {
    NSAssert(_impProviderBlock, nil);
    // Casting IMP to SGI_RSSwizzleOriginalIMP to force user casting.
    return (SGI_RSSwizzleOriginalIMP)_impProviderBlock();
}

@end


#pragma mark └ SGI_RSSwizzle
@implementation SGI_RSSwizzle

static void swizzle(Class classToSwizzle,
    SEL selector,
    SGI_RSSwizzleImpFactoryBlock factoryBlock) {
    ...
    SGI_RSSWizzleImpProvider originalImpProvider = ^IMP {
    ...
    SGI_RSSwizzleInfo *swizzleInfo = [SGI_RSSwizzleInfo new];
...
+ (BOOL)swizzleInstanceMethod:(SEL)selector
                ...
                newImpFactory:(SGI_RSSwizzleImpFactoryBlock)factoryBlock
                         mode:(SGI_RSSwizzleMode)mode
                          key:(const void *)key {
    NSAssert(!(NULL == key && SGI_RSSwizzleModeAlways != mode),
        @"Key may not be NULL if mode is not SGI_RSSwizzleModeAlways.");

    @synchronized(swizzledClassesDictionary()) {
        if (key) {
            NSSet *swizzledClasses = swizzledClassesForKey(key);
            if (mode == SGI_RSSwizzleModeOncePerClass) {
                if ([swizzledClasses containsObject:classToSwizzle]) {
                    return NO;
                }
            } else if (mode == SGI_RSSwizzleModeOncePerClassAndSuperclasses) {
